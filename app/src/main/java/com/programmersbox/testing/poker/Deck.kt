...
fun <T> Iterable<T>.toDeck(listener: (Deck.DeckListenerBuilder<T>.() -> Unit)? = null) =
    Deck(this, listener)

fun <T> Array<T>.toDeck(listener: (Deck.DeckListenerBuilder<T>.() -> Unit)? = null) =
...
class Deck<T> : AbstractDeck<T> {
    ...
    companion object {
        ...
        fun defaultDeck() =
            Deck(
                *Suit.values().map { suit -> (1..13).map { value -> Card(value, suit) } }.flatten()
                    .toTypedArray()
            )
    ...
    class DeckBuilder<T> private constructor() {
        ...
        @Suppress("unused")
        @CardMarker
        fun DeckBuilder<Card>.card(block: CardBuilder.() -> Unit) =
            apply { cardList.add(CardBuilder(block)) }
        ...
        @Suppress("unused")
        @CardMarker
        fun DeckBuilder<Card>.card(value: Int, suit: Suit) =
            apply { cardList.add(Card(value, suit)) }
        ...
        companion object {
             ...
             */
            @DeckMarker
            fun <T> buildDeck(block: DeckBuilder<T>.() -> Unit) =
                DeckBuilder<T>().apply(block).build()
        }
    }
}
