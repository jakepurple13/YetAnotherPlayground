...
abstract class AbstractDeck<T>(cards: Iterable<T> = emptyList()) {
    ...
    protected fun Iterable<T>.toArray() =
        Array(this.count()) { i -> this.toList()[i] as Any } as Array<T>
    ...
    infix fun draw(amount: Int) =
        tryCatch("Deck is Empty") {
            mutableListOf<T>().apply { repeat(amount) { this += draw() } }.toList()
        }
     ...
     */
    infix fun addDeck(otherDeck: AbstractDeck<T>) =
        deckOfCards.addCards(*otherDeck.deckOfCards.toArray())
     ...
     */
    infix fun findCardLocation(card: T) =
        deckOfCards.indexOf(card).let { if (it == -1) null else it }

    /**
     * Draws cards that match [predicate]
     */
    infix fun drawCards(predicate: (T) -> Boolean) =
        findCards(predicate).also { deckOfCards.removeCards(it) }
     ...
     * Removes [card]
     */
    fun remove(vararg card: T) =
        deckOfCards.filter { it in card }.let { deckOfCards.removeCards(it) }
    ...
    fun randomDraw(predicate: (T) -> Boolean = { true }) =
        deckOfCards.filter(predicate)
            .tryCatch("Card Not Found") { it.random().also { c -> remove(c) } }
     ...
     */
    operator fun set(index: Int, card: T) =
        tryCatch("Index: $index, Size: $size") { deckOfCards[index] = card }
    ...
    protected fun split() =
        deckOfCards.subList(0, size / 2).toList() to deckOfCards.subList(size / 2, size).toList()
